...
public class AddDeTasseling extends Activity implements OnItemSelectedListener,
		OnClickListener {
	SQLiteDatabase database;
	String muUid, muUsername, muPassword, muFname, muLname, muEmail, muMobile,
			muUrl, muRole;
	Spinner screw, slocation, sdetasseling;
	...
	String mCrewid, mCrewname, mCrewFname, mCrewLname, mCrewEmail, mCrewMobile,
			mLocationid, mLocationname, mDetasselingno, mDate, mTaskid;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.adddetasseling);
		if (android.os.Build.VERSION.SDK_INT > 9) {
			StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder()
					.permitAll().build();
			StrictMode.setThreadPolicy(policy);
		}
		locationManager = (LocationManager) AddDeTasseling.this
		...
		sdetasseling = (Spinner) findViewById(R.id.spinselectdetasseling);
		...
		sdetasseling.setOnItemSelectedListener(this);
		...
		GetConfigDetails();
		progressDialog = ProgressDialog.show(AddDeTasseling.this,
		...
		// Toast.makeText(getApplicationContext(), String.valueOf(mmonth),
		// Toast.LENGTH_LONG).show();
		// System.err.println(mmonth);
		mday = c.get(Calendar.DAY_OF_MONTH);
		edate.setText(mon + "-" + mday + "-" + myear);

	}

	private void GetDetasselingno() {
		// TODO Auto-generated method stub
		List<String> list = new ArrayList<String>();
		list.add("De-Tasseling No.");
		list.add("1");
		list.add("2");
		list.add("3");
		list.add("4");
		...
		sdetasseling.setAdapter(dataAdapter);
	...
	protected void CrewNotFound(String message) {
		...
		alertDialog.setButton2("OK", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				// TODO Auto-generated method stub
				Intent intent = new Intent(AddDeTasseling.this, AdminHome.class);
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		if (spinner.getId() == R.id.spinselectlocation) {
			...
			System.err.println(mLocationid);
			System.err.println(mLocationname);
			baddtask.setVisibility(View.INVISIBLE);
			bback.setVisibility(View.INVISIBLE);
			bverifylocation.setVisibility(View.VISIBLE);
		}
		if (spinner.getId() == R.id.spinselectdetasseling) {
			mDetasselingno = sdetasseling.getSelectedItem().toString();
	...
	public void onClick(View v) {
		...
		if (v == baddtask) {
			...
			if (mCrewid == null) {
				Toast("Select Crew before adding task");
			} else if (mLocationid == null) {
				Toast("Select Lot before adding task");
			} else if (mDetasselingno.contains("De-Tasseling No.")) {
				Toast("Select De-Tasseling No.");
	...
	private void GetCurrentLocation() {
		// TODO Auto-generated method stub
		gps = new GPSTracker(AddDeTasseling.this);
	...
	protected void AddTasktoServer() {
		...
		try {
			...
			nameValuePairs.add(new BasicNameValuePair("detasselingno",
					mDetasselingno));

			nameValuePairs.add(new BasicNameValuePair("createddate", DateTime));
			nameValuePairs
					.add(new BasicNameValuePair("tasktype", "Detasseling"));
	...
	protected void SendEmailtoCrew() {
		...
		try {
			...
			Toast.makeText(AddDeTasseling.this,
	...
	protected void SuccessAlert() {
		// TODO Auto-generated method stub
		alertDialog = new AlertDialog.Builder(AddDeTasseling.this).create();
		alertDialog.setIcon(R.drawable.success);
