...
public class AddRogueing extends Activity implements OnItemSelectedListener,
		OnClickListener {
	SQLiteDatabase database;
	String muUid, muUsername, muPassword, muFname, muLname, muEmail, muMobile,
			muUrl, muRole;
	Spinner screw, slocation;
	ProgressDialog progressDialog;
	String mResponse, mResponse1, mResponse2;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.addrogueing);
		if (android.os.Build.VERSION.SDK_INT > 9) {
			StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder()
					.permitAll().build();
			StrictMode.setThreadPolicy(policy);
		}
		locationManager = (LocationManager) AddRogueing.this
		...
		GetConfigDetails();
		progressDialog = ProgressDialog.show(AddRogueing.this, "In progress",
				"Please wait");
	...
	protected void CrewNotFound(String message) {
		...
		alertDialog.setButton2("OK", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				// TODO Auto-generated method stub
				Intent intent = new Intent(AddRogueing.this, AdminHome.class);
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		if (spinner.getId() == R.id.spinselectcrew) {
			...
			progressDialog = ProgressDialog.show(AddRogueing.this,
	...
	public void onClick(View v) {
		...
		if (v == bcallcrew) {
			if (mCrewMobile == null) {
				Toast("Select Crew before making call");
			} else {
				try {
	...
	private void GetCurrentLocation() {
		// TODO Auto-generated method stub
		gps = new GPSTracker(AddRogueing.this);
	...
	protected void AddTasktoServer() {
		...
		try {
			...
			ResponseHandler<String> responseHandler = new BasicResponseHandler();
			mResponse2 = httpClient.execute(httpPost, responseHandler);
			mResponse2 = mResponse2.trim().toString();
			runOnUiThread(new Runnable() {
				public void run() {
					progressDialog.dismiss();
					System.err.println(mResponse2);

					if (mResponse2.contains("Error")) {
						ErrorAddingTask();
					} else {
						try {
							jObject = new JSONObject(mResponse2);
							JSONArray menuitemArray = jObject
									.getJSONArray("Task");
	...
	protected void SendEmailtoCrew() {
		...
		try {
			...
			i.putExtra(Intent.EXTRA_SUBJECT, "Rogueing");
			i.putExtra(Intent.EXTRA_TEXT, "Hi " + mCrewFname
					+ "\n\nThe Rogueing (" + mTaskid + ") for the Location: "
					+ mLocationname + "(" + mLocationid
					+ ") is created and the job date is: " + mDate);

			startActivity(Intent.createChooser(i, "Send mail..."));

		} catch (android.content.ActivityNotFoundException ex) {
			Toast.makeText(AddRogueing.this,
	...
	protected void AddtoCalendar() {
		...
		try {
			...
			intent.putExtra("title", "Rogueing");
			intent.putExtra("description", "Rogueing (" + mTaskid
	...
	protected void SuccessAlert() {
		// TODO Auto-generated method stub
		alertDialog = new AlertDialog.Builder(AddRogueing.this).create();
		alertDialog.setIcon(R.drawable.success);
		alertDialog.setMessage("Success");
		alertDialog
				.setMessage("Added Rogueing successfully and the Task Id is:"
						+ mTaskid);
	...
	protected void ErrorAddingTask() {
		...
		alertDialog.setMessage("Error in adding Rogueing");
		alertDialog.setButton("OK", new DialogInterface.OnClickListener() {
